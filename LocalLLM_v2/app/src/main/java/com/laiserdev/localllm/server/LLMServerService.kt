...
class LLMServerService : Service() {
                    ...
                    routing {
                        ...
                        route("/v1") {
                            intercept(ApplicationCallPipeline.Plugins) {
                                ...
                                if (apiKey.isNullOrBlank()) {
                                    call.respond(HttpStatusCode.Unauthorized,
                                        ApiError("Missing X-API-Key header", 401))
                            ...
                            post("/chat") {
                                if (!llmRepo.isLoaded()) {
                                    call.respond(HttpStatusCode.ServiceUnavailable,
                                        ApiError("No model loaded on device", 503)); return@post
                                }
                                val req = call.receive<ApiRequest>()
                                val start = System.currentTimeMillis()
                                llmRepo.generate(req.prompt, req.system ?: "", req.maxTokens)
                                    .fold(
                                        onSuccess = { content ->
                            ...
                            post("/code") {
                                ...
                                val req = call.receive<ApiRequest>()
                                val start = System.currentTimeMillis()
                                llmRepo.generateCode(req.prompt)
                                    .fold(
                                        onSuccess = { content ->
                                            call.respond(ApiResponse(
                                                id = UUID.randomUUID().toString(),
                                        ...
                                        onFailure = { e ->
                                            call.respond(HttpStatusCode.InternalServerError,
                                                ApiError(e.message ?: "Code gen error", 500))
                                        }
                                    )
                            }
    ...
    private fun buildNotification() =
        NotificationCompat.Builder(this, LocalLLMApp.CHANNEL_SERVER)
            ...
            .setContentText("localhost:$PORT  •  Tap to open")
